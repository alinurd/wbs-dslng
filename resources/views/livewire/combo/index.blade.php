<div class="space-y-4">
    ...
    <div class="bg-white p-4 rounded shadow border border-gray-200">
        <div class="mb-4">
            <x-input type="text" wire:model.debounce.500ms="search" placeholder="Cari Kelompok, Data, Param Str..."
                    ...
                    @forelse($dataList as $index => $p)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="border p-2 text-center space-x-2">
                                @if ($CanEdit)
                                    <x-button wire:click="openModal({{ $p->id }})"
                                        class="text-blue-600  hover:underline p-0">Edit</x-button>
                                @endif
                                @if ($CanDelete)
                                    <x-button wire:click="delete({{ $p->id }})"
                                        class="text-red-600  hover:underline p-0">Hapus</x-button>
                                @endif
                            </td>
                        </tr>
                    @empty
