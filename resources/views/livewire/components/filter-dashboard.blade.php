<div class="bg-white rounded-2xl shadow-lg p-6 mb-6">
    <div class="flex items-center justify-between mb-4">
        ...
        <button wire:click="resetFilters" 
            ...
            <i class="fas fa-undo mr-1"></i>{{ __('global.reset_filter') }}
        </button>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div>
            ...
            <select wire:model.live="tahunFilter" 
                ...
                <option value="">{{ __('global.semua') }}</option>
        ...
        <div class="lg:col-span-2">
            <label class="block text-sm font-medium text-gray-700 mb-1">
                {{ __('global.code_pengaduan') }}
            </label>
            <input type="text" wire:model.live="codePengaduanFilter" 
                   placeholder="{{ __('global.placeholder_code_pengaduan') }}"
                   class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
        </div>
    </div>
    ...
    <div class="mt-4 flex flex-wrap gap-2">
        ...
        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
            {{ __('global.jenis') }}:
            {{ $this->getJenisPengaduanOptions()[$jenisPengaduanFilter] ?? $jenisPengaduanFilter }}
        ...
        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
            {{ __('global.direktorat') }}: {{ $this->getDirektoratName($direktoratFilter) }}
        ...
        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
            {{ __('global.status') }}: {{ $this->getComboById($statusFilter) }}
        ...
        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-cyan-100 text-cyan-800">
            {{ __('global.forward') }}: {{ $this->getComboById($fwdToFilter) }}
        ...
        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-teal-100 text-teal-800">
            {{ __('global.code') }}: {{ ucfirst(str_replace('_', ' ', $codePengaduanFilter)) }}
