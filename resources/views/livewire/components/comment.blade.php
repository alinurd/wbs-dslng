...
@if ($show)
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-white/80 mt-1">Status: <span
                                        class="font-semibold">{{ $data['Status'] ?? 'Pending' }}</span></p>
                            </div>
                        </div>
    ...
    @php
        ...
        $messageText = preg_replace_callback(
            '/@([a-zA-Z0-9_]+)/',
            function($matches) {
                return '<span class=" text-blue-300 rounded underline underline-offset-1">@' . $matches[1] . '</span>';
            },
  ...
  <!-- Mention Dropdown - Posisi fixed di atas semua element -->
 <!-- Mention Dropdown -->
@if ($showMentionDropdown && count($mentionUsers) > 0)
    <div class="mention-dropdown">
        <div class="sticky top-0 p-2 border-b border-gray-200 bg-gray-50">
            ...
            <p class="text-xs text-gray-500 mt-1">Tekan ↑↓ untuk navigasi, Enter untuk memilih</p>
        </div>
        <div class="divide-y divide-gray-100">
            @foreach ($mentionUsers as $index => $user)
                <button type="button"
                    wire:click="selectMentionUser({{ $index }})"
...
@endif
 ...
 <div class="flex-1 relative">
    <textarea 
        ...
        wire:keydown.escape="resetMentionDropdown"
        x-data="{
            init() {
...
<div class="mt-2 text-xs text-gray-500 flex items-center">
    ...
    Gunakan <span class="font-medium">@email</span> untuk mention user. User akan mendapatkan notifikasi.
</div>
        <!-- Error Messages -->
        @error('newMessage')
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', function(e) {
        ...
        if (mentionDropdown && messageInput && window.getComputedStyle(mentionDropdown).display !== 'none') {
            ...
            if (mutation.type === 'attributes' || mutation.type === 'childList') {
                ...
                if (mutation.addedNodes) {
                    mutation.addedNodes.forEach(function(node) {
                        if (node.classList && node.classList.contains('mention-dropdown')) {
...
.mention-dropdown {
    position: fixed;
    bottom: 140px; /* Sesuaikan karena textarea lebih tinggi */
    left: 50%;
