            ...
            <div class="modal-body p-6 max-h-[80vh] overflow-y-auto">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div class="bg-white border border-gray-200 rounded-lg shadow-sm">
                            ...
                            <div class="p-4 space-y-4">
                                @foreach($data as $label => $value)
                                    @if(!in_array($label, ['Struktur Hirarki', 'Hierarchy Structure', 'Children List', 'Daftar Children', 'Jumlah Children']))
                                    <div class="flex flex-col sm:flex-row sm:items-start justify-between border-b border-gray-100 pb-3 last:border-b-0">
                                        ...
                                        <div class="sm:w-3/5">
                                            @if($label === 'Status' || $label === 'Status Lapor' || $label === 'Status Lapor Kerja')
                                                <span class="px-3 py-1 text-sm font-semibold rounded-full {{ $value === 'Aktif' ? 'bg-green-100 text-green-800 border border-green-200' : 'bg-red-100 text-red-800 border border-red-200' }}">
                                                    <i class="fas fa-circle text-xs mr-1 {{ $value === 'Aktif' ? 'text-green-500' : 'text-red-500' }}"></i>
                                                    {{ $value }}
                                                </span>
                                            @elseif($label === 'Deskripsi' || $label === 'Description')
                                                <div class="bg-gray-50 border border-gray-200 rounded p-3">
                                                    <p class="text-sm text-gray-700 whitespace-pre-line leading-relaxed">{{ $value ?? 'Tidak ada deskripsi' }}</p>
                                                </div>
                                                ...
                                                <div class="space-y-1">
                                                    @foreach($value as $item)
                                                        <div class="flex items-center space-x-2">
                                                            ...
                                                            <span class="text-sm text-gray-700">{{ is_array($item) ? json_encode($item) : $item }}</span>
                                                        </div>
                                                    @endforeach
                                                </div>
                                            @else
                    ...
                    <div class="space-y-6">
                        @if($hasHierarchy)
                            @php
                                $hierarchyData = $data['Struktur Hirarki'] ?? $data['Hierarchy Structure'] ?? [];
                            @endphp
                            <div class="bg-white border border-gray-200 rounded-lg shadow-sm">
                                <div class="bg-gradient-to-r from-blue-500 to-blue-600 px-4 py-3 border-b border-blue-600 rounded-t-lg">
                                ...
                                <div class="p-4">
                                                    ...
                                                    <div class="bg-gray-50 border border-gray-200 rounded p-4">
                                                        ...
                                                        <div class="space-y-2">
                                                            @foreach($items as $item)
                                                                <div class="flex items-center space-x-2 p-2 bg-white rounded border border-gray-200 hover:shadow-sm transition-all">
                                    ...
                                    @else
                                        <div class="text-center py-8">
                                            ...
                                            <p class="text-gray-400 text-sm mt-1">Data struktur organisasi tidak tersedia</p>
                                        </div>
                                    @endif
                                </div>
                            </div>
                ...
                <div class="bg-white border border-gray-200 rounded-lg shadow-sm">
                    <div class="p-6 space-y-4">
                        @foreach($data as $label => $value)
                        <div class="flex flex-col sm:flex-row sm:items-start justify-between border-b border-gray-100 pb-4 last:border-b-0">
                            ...
                            <div class="sm:w-3/5">
                                ...
                                @elseif($label === 'Deskripsi' || $label === 'Description')
                                    <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                                        <p class="text-gray-700 whitespace-pre-line leading-relaxed">{{ $value ?? 'Tidak ada deskripsi' }}</p>
                                    </div>
                                @elseif(is_array($value))
                                    <div class="space-y-2">
                                        @foreach($value as $item)
                                            <div class="flex items-center space-x-2">
                                                <i class="fas user text-blue-400 text-sm"></i>
                                                <span class="text-gray-700">{{ is_array($item) ? json_encode($item) : $item }}</span>
                                            </div>
                                        @endforeach
                                    </div>
                                @else
