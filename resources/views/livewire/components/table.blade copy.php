...
<div class="overflow-y-auto border border-gray-200 rounded-lg bg-white shadow-sm transition-all duration-300"
     x-data="{
        dropdownOpen: null,
        ...
        openDropdown(id, event) {
            ...
            this.dropdownPosition = {
                x: rect.right - 192, // Sesuaikan dengan lebar dropdown
                y: rect.bottom + 8
            };
        },
        closeDropdown() {
    ...
    @if (session()->has('success') || session()->has('error'))
        <div class="fixed top-4 right-4 animate-fade-in z-50">
            <div class="p-4 rounded-xl shadow-lg {{ session()->has('success') ? 'bg-green-500' : 'bg-red-500' }} text-white">
                ...
                @foreach ($columns as $column => $label)
                    <th wire:click="{{ $onSort }}('{{ $column }}')"
                        ...
                        <div class="group inline-flex items-center gap-x-2 hover:text-gray-200">
                            {{ $label }}
                            <i class="fas {{ $this->getSortIcon($column) }} text-gray-300 group-hover:text-gray-200 transition-all duration-200"></i>
                        </div>
                    </th>
                @endforeach
            ...
            @forelse($records as $index => $record)
                <tr class="hover:bg-blue-50 transition-all duration-200 animate-fade-in">
                    <td class="px-4 py-3 whitespace-nowrap text-center border-b border-gray-100">
                        <input wire:model.live="{{ $onSelectItem }}" 
                               type="checkbox" 
                               value="{{ $record->id }}"
                    ...
                    @foreach ($columns as $column => $label)
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900 border-b border-gray-100">
                            ...
                            @elseif(str_contains($column, 'created_at') || str_contains($column, 'updated_at') || str_contains($column, 'tanggal_pengaduan'))
                                {{ $record->$column ? $record->$column->format('d/m/Y H:i') : '-' }}
                            @elseif(str_contains($column, 'complien_progress'))
                                <span>progress</span>
                            @elseif(str_contains($column, 'aprv_cco'))
                                <span>aprv_cco</span>
                            @else
                                {{ $record->$column ?? '-' }}
                            @endif
                    ...
                    <td class="px-4 py-3 whitespace-nowrap text-center border-b border-gray-100">
                        <div class="flex justify-center gap-1">
                            ...
                            @else
                                <!-- Tampilkan tombol View langsung untuk admin -->
                                @if($record->roles == 'admin' && ($permissions['view'] ?? false))
                                ...
                                @else
                                    <!-- Main Actions Dropdown untuk role lainnya -->
                                    <div class="relative inline-block text-left">
                                        <button type="button" 
                                                @click="openDropdown('{{ $record->id }}', $event)"
                                                ...
                                                :aria-expanded="dropdownOpen === '{{ $record->id }}'">
                                            Options
                                            <i class="fas fa-chevron-down w-3 h-3 ml-2 transition-transform duration-200"
                                               :class="{ 'rotate-180': dropdownOpen === '{{ $record->id }}' }"></i>
                                        </button>
                                    </div>
                                @endif
                            @endif
    ...
    <template x-teleport="body">
        <div x-show="dropdownOpen" 
             ...
             class="fixed w-48 rounded-lg shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none z-[9999]"
             :style="`left: ${dropdownPosition.x}px; top: ${dropdownPosition.y}px;`"
             x-cloak>
            
            @foreach($records as $record)
                <div x-show="dropdownOpen === '{{ $record->id }}'">
                    <div class="py-1">
                        <!-- View Action -->
                        ...
                        @if ($permissions['act_complien'] ?? false)
                            <button wire:click="addNote({{ $record->id }})" 
                                    @click="closeDropdown()"
                                    class="flex items-center w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 transition-all duration-150 group">
