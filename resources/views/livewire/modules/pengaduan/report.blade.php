            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">
                        {{ __('pengaduan.jenis_pengaduan') }}  <span class="text-red-500">*</span>
                    </label>
                    <select wire:model="jenis_pengaduan_id"
                        ...
                        <option value="">{{ __('pengaduan.pilih_jenis') }}</option>
                        @foreach ($jenisPengaduanList as $p)
                            <option value="{{ $p->id }}">{{ $this->locale=='id'?$p->data_id:$p->data_en }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">
                        {{ __('pengaduan.terlapor') }} <span class="text-red-500">*</span>
                    </label>
                    <input type="text" wire:model="nama_terlapor" placeholder=""
                ...
                <div>
                    ...
                    <select wire:model="direktorat"
                        ...
                        <option value="">{{ __('pengaduan.pilih_direktorat') }}</option>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">
                       {{ __('pengaduan.waktu_kejadian') }} <span class="text-red-500">*</span>
                    </label>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">
                        {{ __('pengaduan.uraian') }} <span class="text-red-500">*</span>
                    </label>
                    <textarea wire:model="uraian" rows="3" placeholder=""
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">
                        {{ __('pengaduan.alamat_kejadian') }}  <span class="text-red-500">*</span>
                    </label>
                    <textarea wire:model="alamat_kejadian" rows="3" placeholder=""
                ...
                <div class="flex flex-col items-center justify-center space-y-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <p class="text-sm text-gray-500 max-w-2xl">
                            Maksimal 100MB per file. <br> Format:
...
@if (isset($temporaryFiles) && count($temporaryFiles) > 0)
    <div class="mt-6">
        <h4 class="text-md font-medium text-gray-700 mb-3">
           {{ __('pengaduan.mengunggah_file') }}({{ count($temporaryFiles) }}):
        </h4>
...
@endif
            ...
            <div class="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                <div class="flex items-start">
                    ...
                    <div class="text-sm text-yellow-700">
                        <p class="font-medium">{{ __('pengaduan.catatan_title') }}:</p>
                        <ul class="list-disc list-inside mt-1 space-y-1">
                              @foreach (__('pengaduan.catatan_list') as $item)
        <li>{{ $item }}</li>
    @endforeach
        ...
        <div class="flex flex-col sm:flex-row justify-between gap-3 pt-4 border-t border-gray-200">
            <button type="button" wire:click="resetForm"
                ...
                <i class="fas fa-redo mr-2"></i>{{ __('pengaduan.reset') }}
            </button>
            @if ($confirmation)
    ...
    <button type="submit"
        ...
        <span wire:loading.remove wire:target="submit">{{ __('pengaduan.kirim') }}</span>
        <span wire:loading wire:target="submit">{{ __('pengaduan.memproses') }}</span>
    </button> 
</div>
            @endif
        </div>
