...
@foreach (array_slice(__('wbs.news.items'), 0, 4) as $news)
    <div class="news-card bg-white rounded-xl shadow-md overflow-hidden hover:scale-[1.02] transition relative">
        <div class="relative">
            <img src="{{ asset($news['image']) }}" alt="{{ $news['title'] }}" class="w-full h-30 object-cover">

            <span class="absolute top-3 left-3 bg-white bg-opacity-80 text-gray-700 text-xs font-medium px-2 py-1 rounded shadow">
                {{ \Carbon\Carbon::parse($news['created'])->format('M d, Y') }}
            </span>

            <span class="absolute top-3 right-3 bg-blue-600 text-white text-xs font-semibold px-3 py-1 rounded-full shadow-md bg-opacity-90">
                {{ $news['category'] }}
            </span>
