<header id="mainHeader" class="sticky-header">
            ...
            <nav class="hidden md:flex items-center gap-8">
                ...
                <div x-data="{ open: false }" class="language-dropdown relative">
                    ...
                    <div x-show="open" x-transition:enter="transition ease-out duration-200"
                        ...
                        {{-- Bahasa Indonesia --}}
                        <button wire:click="changeLanguage('id')" @click="open = false" class="w-full text-left px-4 py-2 flex items-center gap-2 text-sm 
                        ...
                        {{-- English --}}
                        <button wire:click="changeLanguage('en')" @click="open = false"
                            class="w-full text-left px-4 py-2 flex items-center gap-2 text-sm {{ $currentLocale === 'en' ? 'text-blue-600 font-semibold bg-blue-50' : 'text-gray-700 hover:bg-gray-100' }}">
                            🇺🇸 {{ __('wbs.header.en') }}
                                                        @if ($currentLocale === 'en')
                                <i class="fas fa-check ml-auto text-blue-600"></i>
                            @endif
    ...
    <div id="mobileMenu" class="md:hidden bg-white border-t border-gray-200 px-4 py-4 hidden">
        <div class="space-y-3">
            <a {{ route('landing.index') }} class="block text-gray-700 hover:text-blue-600 font-medium py-2 border-b border-gray-100">
                {{ __('wbs.header.home') }}
            </a>
            <a {{ route('register') }} class="block text-gray-700 hover:text-blue-600 font-medium py-2 border-b border-gray-100">
                {{ __('wbs.header.register') }}
            </a>
            <a {{ route('login') }} class="block text-gray-700 hover:text-blue-600 font-medium py-2 border-b border-gray-100">
                {{ __('wbs.header.login') }}
            </a>
