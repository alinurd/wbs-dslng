<?php

return [
    'login' => [
        'title' => 'Masuk',
        'dsc' => 'Masuk ke akun Anda untuk melanjutkan',
        'email' => 'Alamat Email',
        'password' => 'Kata Sandi',
        'remember_me' => 'Ingat Saya',
        'forgot_password' => 'Lupa Kata Sandi?',
        'submit' => 'Masuk',
        'no_account' => 'Belum punya akun?',
        'register_here' => 'Daftar di sini',
    ],

    'register' => [
        'title' => 'Pendaftaran',
        'dsc' => 'Silakan lengkapi formulir di bawah ini untuk membuat akun baru.',
        'username' => 'Nama Pengguna',
        'security_question' => 'Pertanyaan Keamanan',
        'choose_question' => '- Pilih Pertanyaan -',
        'security_questions' => [
            'q1' => 'Apa nama hewan peliharaan pertama Anda?',
            'q2' => 'Apa nama sekolah dasar Anda?',
            'q3' => 'Di mana kota kelahiran Anda?',
            'q4' => 'Apa nama ibu kandung Anda?',
            'q5' => 'Apa makanan favorit masa kecil Anda?',
        ],
        'answer' => 'Jawaban',
        'password' => 'Kata Sandi',
        'password_confirmation' => 'Konfirmasi Kata Sandi',
        'full_name' => 'Nama Lengkap',
        'email' => 'Alamat Email',
        'id_number' => 'No. ID/KTP',
        'contact_details' => 'Detail Kontak',
        'phone' => 'Nomor Telepon',
        'type_of_reporter' => 'Jenis Pelapor',
        'employee' => 'Karyawan',
        'non_employee' => 'Bukan Karyawan',
        
        'confirmation_text' => 'Saya dengan ini mengkonfirmasi bahwa setiap dan semua data serta informasi yang dikirimkan akurat dan/atau diperoleh apa adanya dan tidak dibuat-buat atau dipalsukan berdasarkan niat buruk.',
        'submit' => 'Daftar',
        'already_registered' => 'Sudah terdaftar?',
        'login_here' => 'Masuk di sini',
        'note' => 'Catatan: Form Bertanda Tangan (*) Wajib Diisi',
        'detail' => 'Kontak Detail',
    ],

    'verification_code' => 'Kode Verifikasi',
    'validation' => [
        'required' => 'Field ini wajib diisi',
        'email' => 'Format email tidak valid',
        'min' => 'Panjang minimal :min karakter',
        'confirmed' => 'Konfirmasi tidak cocok',
        'unique' => 'Data sudah digunakan',
    ],

    'password' => [
        'reset' => [
            'title' => 'Reset Kata Sandi',
            'email' => 'Email',
            'password' => 'Kata Sandi Baru',
            'confirm_password' => 'Konfirmasi Kata Sandi Baru',
            'submit' => 'Reset Kata Sandi',
        ],
        'forgot' => [
            'title' => 'Lupa Kata Sandi',
            'email' => 'Email',
            'submit' => 'Kirim Link Reset',
            'back_to_login' => 'Kembali ke halaman login',
        ],
    ],

    'password' => 'Password',
        
        'username' => 'Username',
        'username_placeholder' => 'Enter username',
        
    'logout' => 'Keluar',
    'name' => 'Nama Lengkap',
    'confirm_password' => 'Konfirmasi Kata Sandi',
    'reset_password' => 'Atur Ulang Kata Sandi',
    'send_reset_link' => 'Kirim Tautan Reset Kata Sandi',
    'already_have_account' => 'Sudah punya akun?',
    'dont_have_account' => 'Belum punya akun?',
    
    'failed' => 'Kredensial ini tidak cocok dengan data kami.',
    'throttle' => 'Terlalu banyak percobaan login. Silakan coba lagi dalam :seconds detik.',


      'validation' => [
        'required' => ':attribute wajib diisi.',
        'min' => ':attribute minimal :min karakter.',
        'unique' => ':attribute sudah digunakan.',
        'email' => 'Format :attribute tidak valid.',
        'confirmed' => 'Konfirmasi :attribute tidak cocok.',
        'accepted' => 'Anda harus menyetujui :attribute.',
        'invalid_option' => 'Pilihan :attribute tidak valid.',
    ],
     'forgot' => [
        'title' => 'Lupa kata sandi ',
            'dsc'=>' Lupa kata sandi Anda? Tidak masalah. Cukup beri tahu kami alamat email Anda dan kami akan mengirimkan email berisi tautan pengaturan ulang kata sandi yang memungkinkan Anda memilih yang baru.',
    ],

    


];